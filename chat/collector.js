...
async function startCollector(channelId, ownerNickname) {
  ...
  socket.on("event", (payload) => {
    ...
    else if (payload?.event === "STREAM_LEX_CHANGE") {
      const totalLex = Number(payload?.data?.amount);

      if (Number.isFinite(totalLex) && totalLex >= 0) {
        streamLexStore.set(channelId, totalLex);
        console.log("STREAM_LEX_CHANGE:", channelId, totalLex);
      }
    }
