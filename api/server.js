...
function getAuditActionLabel(action) {
  switch (String(action || "")) {
    ...
    case "roulette_refresh":
      return "룰렛 후보 새로고침";
    case "roulette_candidate_add":
      return "룰렛 후보 추가";
    case "roulette_candidate_remove":
      return "룰렛 후보 제거";
    case "roulette_start":
      return "룰렛 시작";
    case "roulette_stop":
...
app.post("/dashboard/api/roulette/create", requireDashboardAuthApi, async (req, res) => {
  try {
    ...
    await writeAdminAuditLog({
      ...
      operatorName: req.authSession?.operatorName,
      action: "roulette_create",
      note: `eventId=${event.id}, candidates=${Number(event.candidateCount || event.candidates?.length || 0)}, liveOnly=${!!event.liveOnly}`
    });
...
app.post("/dashboard/api/roulette/:eventId/candidates/add", requireDashboardAuthApi, async (req, res) => {
  try {
    const eventId = String(req.params.eventId || "").trim();
    const channelId = normalizeChannelId(req.body.channelId);
    ...
    if (!channelId) {
      res.status(400).send({ ok: false, error: "channelId required" });
      return;
    }

    const updated = await rouletteService.addCandidate(eventId, channelId);

    await writeAdminAuditLog({
      req,
      actorRole: req.authSession?.role,
      operatorName: req.authSession?.operatorName,
      action: "roulette_candidate_add",
      note: `eventId=${eventId}, channelId=${channelId}, candidates=${Number(updated?.candidateCount || 0)}`
    });
    ...
    console.error("roulette candidate add error:", err);
    res.status(500).send({ ok: false, error: err.message });
  }
});

app.post("/dashboard/api/roulette/:eventId/candidates/remove", requireDashboardAuthApi, async (req, res) => {
  try {
    const eventId = String(req.params.eventId || "").trim();
    const channelId = normalizeChannelId(req.body.channelId);
    ...
    const updated = await rouletteService.removeCandidate(eventId, channelId);

    await writeAdminAuditLog({
      req,
      actorRole: req.authSession?.role,
      operatorName: req.authSession?.operatorName,
      action: "roulette_candidate_remove",
      note: `eventId=${eventId}, channelId=${channelId}, candidates=${Number(updated?.candidateCount || 0)}`
    });
    ...
    console.error("roulette candidate remove error:", err);
    res.status(500).send({ ok: false, error: err.message });
  }
});
