// const crypto = require("crypto");
// const db = require("../firebase");
// const streamStore = require("../store/streamStore");
// const rouletteEventStore = require("../store/rouletteEventStore");

// async function buildLiveCandidates() {
//   const snapshot = await db.collection("channels").get();
//   const registeredIds = new Set(snapshot.docs.map((doc) => String(doc.id)));
//   const players = streamStore.getAll();
//   const allowAllLiveForTest = process.env.ROULETTE_ALLOW_ALL_LIVE === "true";

//   console.log("[roulette] allowAllLiveForTest =", allowAllLiveForTest);
//   console.log("[roulette] registeredIds =", [...registeredIds]);
//   console.log(
//     "[roulette] livePlayers =",
//     players.map((p) => ({
//       channelId: String(p.channelId),
//       ownerNickname: p.ownerNickname || "",
//       title: p.title || ""
//     }))
//   );

//   const candidates = players
//     .filter((player) => {
//       if (allowAllLiveForTest) {
//         return true;
//       }

//       return registeredIds.has(String(player.channelId));
//     })
//     .map((player) => ({
//       channelId: String(player.channelId || ""),
//       ownerNickname: String(player.ownerNickname || ""),
//       title: String(player.title || ""),
//       streamId: String(player.streamId || ""),
//       startedAt: Number(player.startedAt || 0)
//     }));

//   console.log("[roulette] candidates =", candidates);
//   return candidates;
// }

// function getStopFinishAtMs(event) {
//   const stopAt = Number(event?.stopRequestedAtMs || 0);
//   const duration = Number(event?.stopDurationMs || 0);

//   if (!stopAt || !duration) {
//     return 0;
//   }

//   return stopAt + duration;
// }

// function shouldFinalizeEvent(event) {
//   if (!event || event.status !== "stopping") {
//     return false;
//   }

//   if (event.manualStopRequested !== true) {
//     return false;
//   }

//   const finishAtMs = getStopFinishAtMs(event);
//   return finishAtMs > 0 && Date.now() >= finishAtMs;
// }

// async function finalizeEventIfNeeded(event) {
//   if (!shouldFinalizeEvent(event)) {
//     return event;
//   }

//   return rouletteEventStore.patchEvent(event.id, {
//     status: "finished",
//     finishedAtMs: Date.now()
//   });
// }

// async function getNormalizedEvent(eventId) {
//   const event = await rouletteEventStore.getEventById(eventId);
//   return finalizeEventIfNeeded(event);
// }

// async function ensureReadyCandidates(eventId, existingCandidates) {
//   let candidates = Array.isArray(existingCandidates) ? existingCandidates : [];

//   if (candidates.length) {
//     return candidates;
//   }

//   candidates = await buildLiveCandidates();

//   if (!candidates.length) {
//     throw new Error("candidate_empty");
//   }

//   await rouletteEventStore.patchEvent(eventId, {
//     candidates,
//     candidateCount: candidates.length
//   });

//   return candidates;
// }

// function chooseWinnerPatch(candidates) {
//   if (!Array.isArray(candidates) || !candidates.length) {
//     throw new Error("candidate_empty");
//   }

//   const targetIndex = crypto.randomInt(candidates.length);
//   const winner = candidates[targetIndex];

//   return {
//     status: "stopping",
//     manualStopRequested: true,
//     stopRequestedAtMs: Date.now(),
//     stopDurationMs: 6500,
//     finalRotationTurns: 8 + crypto.randomInt(4),
//     targetIndex,
//     winnerChannelId: String(winner.channelId || ""),
//     winnerNickname: String(winner.ownerNickname || winner.channelId || ""),
//     drawCandidateCount: candidates.length,
//     finishedAtMs: 0
//   };
// }

// async function createRouletteEvent({ title, sponsorName, prizeText, createdBy }) {
//   const candidates = await buildLiveCandidates();

//   if (!candidates.length) {
//     throw new Error("candidate_empty");
//   }

//   const shareToken = crypto.randomBytes(16).toString("hex");

//   const event = await rouletteEventStore.createEvent({
//     title,
//     sponsorName,
//     prizeText,
//     createdBy,
//     shareToken,
//     status: "ready",
//     candidates
//   });

//   await rouletteEventStore.setCurrentEventId(event.id);
//   return event;
// }

// async function refreshCandidates(eventId) {
//   const event = await getNormalizedEvent(eventId);

//   if (!event) {
//     throw new Error("event_not_found");
//   }

//   if (event.status !== "ready") {
//     throw new Error("event_not_refreshable");
//   }

//   const candidates = await buildLiveCandidates();

//   return rouletteEventStore.patchEvent(eventId, {
//     candidates,
//     candidateCount: candidates.length,
//     winnerChannelId: "",
//     winnerNickname: "",
//     targetIndex: -1,
//     stopRequestedAtMs: 0,
//     stopDurationMs: 6500,
//     finalRotationTurns: 0,
//     manualStopRequested: false,
//     finishedAtMs: 0,
//     drawCandidateCount: 0
//   });
// }

// async function startEvent(eventId) {
//   const event = await getNormalizedEvent(eventId);

//   if (!event) {
//     throw new Error("event_not_found");
//   }

//   if (event.status !== "ready") {
//     throw new Error("event_not_startable");
//   }

//   const candidates = await ensureReadyCandidates(eventId, event.candidates);

//   console.log("[roulette] startEvent requested:", {
//     eventId,
//     prevStatus: event.status,
//     candidateCount: candidates.length
//   });

//   return rouletteEventStore.patchEvent(eventId, {
//     status: "spinning",
//     startedAtMs: Date.now(),
//     stopRequestedAtMs: 0,
//     stopDurationMs: 6500,
//     finalRotationTurns: 0,
//     targetIndex: -1,
//     winnerChannelId: "",
//     winnerNickname: "",
//     manualStopRequested: false,
//     candidates,
//     candidateCount: candidates.length,
//     drawCandidateCount: candidates.length,
//     finishedAtMs: 0
//   });
// }

// async function stopEvent(eventId) {
//   const event = await getNormalizedEvent(eventId);

//   if (!event) {
//     throw new Error("event_not_found");
//   }

//   if (event.status !== "spinning") {
//     throw new Error("event_not_spinning");
//   }

//   console.log("[roulette] stopEvent requested:", {
//     eventId,
//     prevStatus: event.status,
//     candidateCount: Array.isArray(event.candidates) ? event.candidates.length : 0
//   });

//   const candidates = Array.isArray(event.candidates) ? event.candidates : [];
//   return rouletteEventStore.patchEvent(eventId, chooseWinnerPatch(candidates));
// }

// async function redrawEvent(eventId) {
//   const event = await getNormalizedEvent(eventId);

//   if (!event) {
//     throw new Error("event_not_found");
//   }

//   if (event.status !== "finished") {
//     throw new Error("event_not_redrawable");
//   }

//   console.log("[roulette] stopEvent requested:", {
//     eventId,
//     prevStatus: event.status,
//     candidateCount: Array.isArray(event.candidates) ? event.candidates.length : 0
//   });

//   const candidates = Array.isArray(event.candidates) ? event.candidates : [];
//   return rouletteEventStore.patchEvent(eventId, chooseWinnerPatch(candidates));
// }

// async function closeEvent(eventId) {
//   const event = await getNormalizedEvent(eventId);

//   if (!event) {
//     throw new Error("event_not_found");
//   }

//   if (event.status !== "ready" && event.status !== "finished") {
//     throw new Error("event_not_closable");
//   }

//   const updated = await rouletteEventStore.patchEvent(eventId, {
//     status: "closed",
//     manualStopRequested: false,
//     closedAtMs: Date.now()
//   });

//   await rouletteEventStore.clearCurrentEventId(eventId);
//   return updated;
// }

// function getDisplayStatus(event) {
//   if (!event) {
//     return "closed";
//   }

//   if (shouldFinalizeEvent(event)) {
//     return "finished";
//   }

//   return String(event.status || "ready");
// }

// async function getPublicState(eventId) {
//   const event = await getNormalizedEvent(eventId);

//   if (!event) {
//     return null;
//   }

//   return {
//     ...event,
//     displayStatus: getDisplayStatus(event)
//   };
// }

// async function validateViewerToken(eventId, token) {
//   const event = await getNormalizedEvent(eventId);

//   if (!event) {
//     return null;
//   }

//   if (String(event.shareToken || "") !== String(token || "")) {
//     return null;
//   }

//   return event;
// }

// module.exports = {
//   buildLiveCandidates,
//   createRouletteEvent,
//   refreshCandidates,
//   startEvent,
//   stopEvent,
//   redrawEvent,
//   closeEvent,
//   getPublicState,
//   validateViewerToken
// };
const crypto = require("crypto");
const db = require("../firebase");
const streamStore = require("../store/streamStore");
const profileCache = require("../store/profileCache");
const rouletteEventStore = require("../store/rouletteEventStore");

function normalizeStartedAt(value) {
  if (value === null || value === undefined || value === "") {
    return 0;
  }

  if (typeof value === "number" && Number.isFinite(value)) {
    if (value > 0 && value < 1e12) {
      return value * 1000;
    }

    return value;
  }

  const text = String(value).trim();

  if (!text) {
    return 0;
  }

  if (/^\d+$/.test(text)) {
    const asNumber = Number(text);
    if (Number.isFinite(asNumber)) {
      return asNumber > 0 && asNumber < 1e12 ? asNumber * 1000 : asNumber;
    }
  }

  const parsed = Date.parse(text);
  return Number.isFinite(parsed) ? parsed : 0;
}

function sanitizeStoredCandidate(candidate, { manual = false } = {}) {
  const channelId = String(candidate?.channelId || "").trim();

  if (!channelId) {
    return null;
  }

  return {
    channelId,
    streamId: String(candidate?.streamId || ""),
    title: String(candidate?.title || ""),
    startedAtMs: Number(candidate?.startedAtMs || normalizeStartedAt(candidate?.startedAt) || 0),
    isLive: !!candidate?.isLive,
    manual: manual || !!candidate?.manual
  };
}

function dedupeCandidates(candidates) {
  const map = new Map();

  for (const raw of Array.isArray(candidates) ? candidates : []) {
    const candidate = sanitizeStoredCandidate(raw);

    if (!candidate) {
      continue;
    }

    map.set(candidate.channelId, candidate);
  }

  return [...map.values()];
}

async function getRegisteredChannelIds() {
  const snapshot = await db.collection("channels").get();
  const ids = [];

  snapshot.forEach((doc) => {
    const channelId = String(doc.id || "").trim();
    if (channelId) {
      ids.push(channelId);
    }
  });

  ids.sort((a, b) => a.localeCompare(b, "ko"));
  return ids;
}

function getLivePlayerMap() {
  const players = streamStore.getAll();
  const map = new Map();

  for (const player of players) {
    const channelId = String(player?.channelId || "").trim();

    if (!channelId) {
      continue;
    }

    map.set(channelId, {
      channelId,
      streamId: String(player?.streamId || ""),
      title: String(player?.title || ""),
      startedAtMs: normalizeStartedAt(player?.startedAt),
      ownerNickname: String(player?.ownerNickname || ""),
      isLive: true
    });
  }

  return map;
}

function buildStoredCandidateFromLivePlayer(player, { manual = false } = {}) {
  return sanitizeStoredCandidate({
    channelId: player?.channelId,
    streamId: player?.streamId,
    title: player?.title,
    startedAtMs: player?.startedAtMs,
    isLive: true,
    manual
  }, { manual });
}

async function buildStoredCandidateByChannelId(channelId, livePlayerMap, { manual = false } = {}) {
  const normalizedId = String(channelId || "").trim();

  if (!normalizedId) {
    throw new Error("channelId required");
  }

  const livePlayer = livePlayerMap.get(normalizedId);

  if (livePlayer) {
    return buildStoredCandidateFromLivePlayer(livePlayer, { manual });
  }

  await profileCache.getNickname(normalizedId);

  return sanitizeStoredCandidate({
    channelId: normalizedId,
    streamId: "",
    title: "",
    startedAtMs: 0,
    isLive: false,
    manual
  }, { manual });
}

async function buildRegisteredCandidates({ liveOnly = false } = {}) {
  const registeredIds = await getRegisteredChannelIds();
  const livePlayerMap = getLivePlayerMap();

  if (liveOnly) {
    return registeredIds
      .map((channelId) => livePlayerMap.get(channelId))
      .filter(Boolean)
      .map((player) => buildStoredCandidateFromLivePlayer(player))
      .filter(Boolean);
  }

  const candidates = await Promise.all(
    registeredIds.map((channelId) => buildStoredCandidateByChannelId(channelId, livePlayerMap))
  );

  return dedupeCandidates(candidates);
}

async function hydrateCandidateForDisplay(candidate, livePlayerMap) {
  const stored = sanitizeStoredCandidate(candidate);

  if (!stored) {
    return null;
  }

  const livePlayer = livePlayerMap.get(stored.channelId);
  const ownerNickname = livePlayer?.ownerNickname || await profileCache.getNickname(stored.channelId);

  return {
    ...stored,
    ownerNickname: String(ownerNickname || stored.channelId),
    streamId: livePlayer?.streamId || stored.streamId || "",
    title: livePlayer?.title || stored.title || "",
    startedAtMs: livePlayer?.startedAtMs || stored.startedAtMs || 0,
    isLive: !!livePlayer,
    startedAt: (livePlayer?.startedAtMs || stored.startedAtMs || 0) || 0
  };
}

async function hydrateEventDisplayData(event) {
  if (!event) {
    return null;
  }

  const livePlayerMap = getLivePlayerMap();
  const storedCandidates = dedupeCandidates(event.candidates);
  const candidates = (await Promise.all(
    storedCandidates.map((candidate) => hydrateCandidateForDisplay(candidate, livePlayerMap))
  )).filter(Boolean);

  let winnerNickname = "";
  const winnerChannelId = String(event.winnerChannelId || "").trim();

  if (winnerChannelId) {
    const winnerLivePlayer = livePlayerMap.get(winnerChannelId);
    winnerNickname = winnerLivePlayer?.ownerNickname || await profileCache.getNickname(winnerChannelId);
  }

  return {
    ...event,
    liveOnly: !!event.liveOnly,
    candidateModeText: event.liveOnly ? "현재 접속 BJ만" : "channels 등록 BJ 전체",
    candidates,
    candidateCount: candidates.length,
    drawCandidateCount: Number(event.drawCandidateCount || candidates.length || 0),
    winnerNickname: String(winnerNickname || "")
  };
}

function getStopFinishAtMs(event) {
  const stopAt = Number(event?.stopRequestedAtMs || 0);
  const duration = Number(event?.stopDurationMs || 0);

  if (!stopAt || !duration) {
    return 0;
  }

  return stopAt + duration;
}

function shouldFinalizeEvent(event) {
  if (!event || event.status !== "stopping") {
    return false;
  }

  if (event.manualStopRequested !== true) {
    return false;
  }

  const finishAtMs = getStopFinishAtMs(event);
  return finishAtMs > 0 && Date.now() >= finishAtMs;
}

async function finalizeEventIfNeeded(event) {
  if (!shouldFinalizeEvent(event)) {
    return event;
  }

  return rouletteEventStore.patchEvent(event.id, {
    status: "finished",
    finishedAtMs: Date.now()
  });
}

async function getNormalizedEvent(eventId) {
  const event = await rouletteEventStore.getEventById(eventId);
  return finalizeEventIfNeeded(event);
}

function ensureReadyCandidates(existingCandidates) {
  const candidates = dedupeCandidates(existingCandidates);

  if (!candidates.length) {
    throw new Error("candidate_empty");
  }

  return candidates;
}

function chooseWinnerPatch(candidates) {
  if (!Array.isArray(candidates) || !candidates.length) {
    throw new Error("candidate_empty");
  }

  const targetIndex = crypto.randomInt(candidates.length);
  const winner = candidates[targetIndex];

  return {
    status: "stopping",
    manualStopRequested: true,
    stopRequestedAtMs: Date.now(),
    stopDurationMs: 6500,
    finalRotationTurns: 8 + crypto.randomInt(4),
    targetIndex,
    winnerChannelId: String(winner.channelId || ""),
    drawCandidateCount: candidates.length,
    finishedAtMs: 0
  };
}

async function createRouletteEvent({ title, sponsorName, prizeText, createdBy, liveOnly = false }) {
  const candidates = await buildRegisteredCandidates({ liveOnly });

  if (!candidates.length) {
    throw new Error("candidate_empty");
  }

  const shareToken = crypto.randomBytes(16).toString("hex");

  const event = await rouletteEventStore.createEvent({
    title,
    sponsorName,
    prizeText,
    createdBy,
    shareToken,
    status: "ready",
    liveOnly,
    candidates
  });

  await rouletteEventStore.setCurrentEventId(event.id);
  return event;
}

async function refreshCandidates(eventId) {
  const event = await getNormalizedEvent(eventId);

  if (!event) {
    throw new Error("event_not_found");
  }

  if (event.status !== "ready") {
    throw new Error("event_not_refreshable");
  }

  const candidates = await buildRegisteredCandidates({
    liveOnly: !!event.liveOnly
  });

  return rouletteEventStore.patchEvent(eventId, {
    candidates,
    candidateCount: candidates.length,
    winnerChannelId: "",
    targetIndex: -1,
    stopRequestedAtMs: 0,
    stopDurationMs: 6500,
    finalRotationTurns: 0,
    manualStopRequested: false,
    finishedAtMs: 0,
    drawCandidateCount: 0
  });
}

async function ensureEditableEventForCandidates(eventId) {
  const event = await getNormalizedEvent(eventId);

  if (!event) {
    throw new Error("event_not_found");
  }

  if (event.status !== "ready") {
    throw new Error("event_candidate_not_editable");
  }

  return event;
}

async function addCandidate(eventId, channelId) {
  const event = await ensureEditableEventForCandidates(eventId);
  const normalizedChannelId = String(channelId || "").trim();

  if (!normalizedChannelId) {
    throw new Error("channelId required");
  }

  const currentCandidates = dedupeCandidates(event.candidates);

  if (currentCandidates.some((candidate) => candidate.channelId === normalizedChannelId)) {
    return event;
  }

  const livePlayerMap = getLivePlayerMap();
  const addedCandidate = await buildStoredCandidateByChannelId(normalizedChannelId, livePlayerMap, { manual: true });
  const nextCandidates = dedupeCandidates([...currentCandidates, addedCandidate]);

  return rouletteEventStore.patchEvent(eventId, {
    candidates: nextCandidates,
    candidateCount: nextCandidates.length,
    drawCandidateCount: 0,
    winnerChannelId: "",
    targetIndex: -1,
    finishedAtMs: 0,
    manualStopRequested: false
  });
}

async function removeCandidate(eventId, channelId) {
  const event = await ensureEditableEventForCandidates(eventId);
  const normalizedChannelId = String(channelId || "").trim();

  if (!normalizedChannelId) {
    throw new Error("channelId required");
  }

  const currentCandidates = dedupeCandidates(event.candidates);
  const nextCandidates = currentCandidates.filter((candidate) => candidate.channelId !== normalizedChannelId);

  if (nextCandidates.length === currentCandidates.length) {
    return event;
  }

  return rouletteEventStore.patchEvent(eventId, {
    candidates: nextCandidates,
    candidateCount: nextCandidates.length,
    drawCandidateCount: 0,
    winnerChannelId: "",
    targetIndex: -1,
    finishedAtMs: 0,
    manualStopRequested: false
  });
}

async function startEvent(eventId) {
  const event = await getNormalizedEvent(eventId);

  if (!event) {
    throw new Error("event_not_found");
  }

  if (event.status !== "ready") {
    throw new Error("event_not_startable");
  }

  const candidates = ensureReadyCandidates(event.candidates);

  console.log("[roulette] startEvent requested:", {
    eventId,
    prevStatus: event.status,
    candidateCount: candidates.length,
    liveOnly: !!event.liveOnly
  });

  return rouletteEventStore.patchEvent(eventId, {
    status: "spinning",
    startedAtMs: Date.now(),
    stopRequestedAtMs: 0,
    stopDurationMs: 6500,
    finalRotationTurns: 0,
    targetIndex: -1,
    winnerChannelId: "",
    manualStopRequested: false,
    candidates,
    candidateCount: candidates.length,
    drawCandidateCount: candidates.length,
    finishedAtMs: 0
  });
}

async function stopEvent(eventId) {
  const event = await getNormalizedEvent(eventId);

  if (!event) {
    throw new Error("event_not_found");
  }

  if (event.status !== "spinning") {
    throw new Error("event_not_spinning");
  }

  console.log("[roulette] stopEvent requested:", {
    eventId,
    prevStatus: event.status,
    candidateCount: Array.isArray(event.candidates) ? event.candidates.length : 0
  });

  const candidates = dedupeCandidates(event.candidates);
  return rouletteEventStore.patchEvent(eventId, chooseWinnerPatch(candidates));
}

async function redrawEvent(eventId) {
  const event = await getNormalizedEvent(eventId);

  if (!event) {
    throw new Error("event_not_found");
  }

  if (event.status !== "finished") {
    throw new Error("event_not_redrawable");
  }

  console.log("[roulette] redrawEvent requested:", {
    eventId,
    prevStatus: event.status,
    candidateCount: Array.isArray(event.candidates) ? event.candidates.length : 0
  });

  const candidates = dedupeCandidates(event.candidates);
  return rouletteEventStore.patchEvent(eventId, chooseWinnerPatch(candidates));
}

async function closeEvent(eventId) {
  const event = await getNormalizedEvent(eventId);

  if (!event) {
    throw new Error("event_not_found");
  }

  if (event.status !== "ready" && event.status !== "finished") {
    throw new Error("event_not_closable");
  }

  const updated = await rouletteEventStore.patchEvent(eventId, {
    status: "closed",
    manualStopRequested: false,
    closedAtMs: Date.now()
  });

  await rouletteEventStore.clearCurrentEventId(eventId);
  return updated;
}

function getDisplayStatus(event) {
  if (!event) {
    return "closed";
  }

  if (shouldFinalizeEvent(event)) {
    return "finished";
  }

  return String(event.status || "ready");
}

async function getPublicState(eventId) {
  const event = await getNormalizedEvent(eventId);

  if (!event) {
    return null;
  }

  const hydrated = await hydrateEventDisplayData(event);

  return {
    ...hydrated,
    displayStatus: getDisplayStatus(event)
  };
}

async function validateViewerToken(eventId, token) {
  const event = await getNormalizedEvent(eventId);

  if (!event) {
    return null;
  }

  if (String(event.shareToken || "") !== String(token || "")) {
    return null;
  }

  return event;
}

module.exports = {
  buildRegisteredCandidates,
  createRouletteEvent,
  refreshCandidates,
  addCandidate,
  removeCandidate,
  startEvent,
  stopEvent,
  redrawEvent,
  closeEvent,
  getPublicState,
  validateViewerToken
};