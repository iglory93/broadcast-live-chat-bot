...
`;
  ...
  try {
    ...
    if (command === "업타임") {
      const streamInfo = streamStore.get(channelId);
      const startedAtMs = parseStartedAtMs(streamInfo?.startedAt);

      if (!startedAtMs) {
        await sendChat(channelId, "현재 방송 시작 시간을 찾을 수 없습니다.");
        return;
      }

      const uptimeText = formatUptimeFromStartedAt(startedAtMs);
      ...
      await sendChat(channelId, `⏱ 방송 시작 후 ${uptimeText} 지났어요.`);
      return;
    }
...
function formatUptimeFromStartedAt(startedAtMs) {
  const now = Date.now();
  const diffMs = now - Number(startedAtMs || 0);
